...
class TodayPageState extends State<TodayPage> with WidgetsBindingObserver {
  ...
  void _syncTodayCardAuthority() {
    const isPreview = SharedPreferencesUtil.isTodayDesignPreview;
    final uid = isPreview ? 'today-card-preview' : SharedPreferencesUtil().uid;
    final isReady = isPreview || _provisioningProvider?.isOperational == true;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      unawaited(_todayCardController.onResumed());
    }
  }
