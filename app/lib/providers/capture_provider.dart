...
class _DeviceCaptureAttempt {
  _DeviceCaptureAttempt({required this.id, required this.deviceId, required this.accountGeneration});
...
class CaptureProvider extends ChangeNotifier
  ...
  bool get transcriptServiceReady {
    ...
    return _transcriptServiceReady && activeSocketReady;
  }
  ...
  Future<void> _failDeviceCaptureSession(
    _DeviceCaptureSession session,
    String reason, {
    CaptureDiagnosticFailure failure = CaptureDiagnosticFailure.necklaceConnectionUnavailable,
  }) async {
  ...
  void _processVoiceCommandBytes(String deviceId, List<List<int>> data) async {
    final session = _deviceCaptureSession;
    if (session == null || !_isDeviceCaptureCurrent(session)) return;
    if (data.isEmpty) {
  ...
  bool _recoverDeviceCaptureSocket(
    ...
    final recovery = hasServerMoment
        ? _serializeDeviceConversationBoundary(reason: reason, initialFrame: frame, allowLostTransportRecovery: true)
  ...
  Future<bool> _replayDeviceSocketReplacementBuffer(
    ...
    for (final frame in buffer.frames) {
      if (!await _sendDeviceFrame(session, replacement, frame)) return false;
    }
    buffer.clear();
    return true;
  }
  ...
  Future<bool> _replaceDeviceCaptureSocket(
    ...
    try {
      ...
      if (!mayConnectReplacement) {
        await _failDeviceCaptureSession(
          session,
          'Necklace transcription replacement authority is ambiguous',
          failure: deniedReplacementFailure,
        );
        return false;
      }
      if (!_isDeviceCaptureCurrent(session)) return false;
  ...
  Future<bool> _ensureCurrentCaptureConsentAuthority() =>
      ...
      ensureCaptureConsentAuthority(
        ...
        refreshAuthority: (uid, receiptId, serverDecidedAt) => EllaAiConsentService().refreshActiveSessionAuthority(
          ...
          expectedServerDecidedAt: serverDecidedAt,
        ),
      );
  ...
  Future<bool> _finalizeCurrentDeviceConversationAndContinue({
    ...
    try {
      ...
      if (operation.captureProtocolAuthority == null) {
        await _failDeviceCaptureSession(
          session,
          'Necklace moment cannot finalize without capture protocol authority',
      ...
      try {
        final replaced = await _replaceDeviceCaptureSocket(
          ...
          afterOldSocketStopped: () async {
            ...
            finalized = await _forceProcessingConversationId(
              exactConversationId,
              operation,
              requireCaptureProtocolAuthority: true,
            );
          ...
          deniedReplacementFailure: CaptureDiagnosticFailure.finalizationFailed,
        );
  ...
  void onClosed([int? closeCode]) {
    ...
    if (!(_socket?.hasActiveSessionAuthority ?? SharedPreferencesUtil().aiConsentAccepted)) {
      _keepAliveTimer?.cancel();
      return;
    }
    _startKeepAliveServices();
  ...
  Future<FinalCapturableContentResult> _awaitFinalCapturableContent(
    ...
    for (var attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      if (loadResult == _InProgressConversationLoadResult.loaded) {
        ...
        final expectedVisibleTranscript = _mergeVisibleTranscriptEvidence(visibleBeforeRefresh, liveTranscriptAtCommit);
        final expectedVisiblePhotos = _mergeVisiblePhotoEvidence(visiblePhotosBeforeRefresh, livePhotosAtCommit);
    ...
    for (final segment in [...beforeRefresh, ...afterRefresh]) {
      ...
      if (index == -1) {
        merged.add(segment);
          ...
          _normalizedTranscriptText(segment.text) != _normalizedTranscriptText(merged[index].text)) {
  ...
  bool _authoritativeTranscriptCovers(List<TranscriptSegment> authoritative, List<TranscriptSegment> visible) =>
      visible.every(
        (visibleSegment) => authoritative.any(
          (authoritativeSegment) =>
              ...
              _transcriptTextCovers(authoritativeSegment.text, visibleSegment.text) &&
              authoritativeSegment.end + 0.001 >= visibleSegment.end,
        ),
      );

  bool _authoritativeTranscriptIdentityCovers(List<TranscriptSegment> authoritative, List<TranscriptSegment> visible) =>
      visible.every(
        (visibleSegment) => authoritative.any(
          (authoritativeSegment) =>
              _sameTranscriptSegment(authoritativeSegment, visibleSegment) &&
              authoritativeSegment.text.trim().isNotEmpty &&
              _transcriptExtentCovers(authoritativeSegment.text, visibleSegment.text) &&
              authoritativeSegment.end + 0.001 >= visibleSegment.end,
        ),
      );
  ...
  bool _transcriptExtentCovers(String authoritative, String visible) {
    final normalizedAuthoritative = _normalizedTranscriptText(authoritative);
    final normalizedVisible = _normalizedTranscriptText(visible);
    if (normalizedVisible.isEmpty) return true;

    return _transcriptWordCount(normalizedAuthoritative) >= _transcriptWordCount(normalizedVisible) &&
        _nonWhitespaceRuneCount(normalizedAuthoritative) >= _nonWhitespaceRuneCount(normalizedVisible);
  }

  int _transcriptWordCount(String normalizedText) {
    return normalizedText.isEmpty ? 0 : normalizedText.split(' ').length;
  }

  int _nonWhitespaceRuneCount(String normalizedText) {
    return normalizedText.replaceAll(' ', '').runes.length;
  }
  ...
  bool _transcriptTextCovers(String authoritative, String visible) {
    ...
    final normalizedVisible = _normalizedTranscriptText(visible);
    if (normalizedVisible.isEmpty) return true;
    return normalizedAuthoritative == normalizedVisible || normalizedAuthoritative.startsWith('$normalizedVisible ');
  }
    ...
    for (final visibleSegment in visible) {
      ...
      if (index == -1) {
        merged.add(visibleSegment);
          ...
          !_transcriptTextCovers(merged[index].text, visibleSegment.text)) {
  ...
  List<ConversationPhoto> _capturablePhotos(List<ConversationPhoto> candidates) =>
      candidates.where((photo) => !photo.discarded && photo.base64.trim().isNotEmpty).toList();

  List<ConversationPhoto> _mergeVisiblePhotoEvidence(
  ...
  Future<bool> _forceProcessingConversationId(
    ...
    bool requireCaptureProtocolAuthority = false,
  }) async {
    final conversations = conversationProvider;
    if (conversations == null || !activeOperation.isCurrent) return false;
    final captureAuthority = activeOperation.captureProtocolAuthority;
    if (requireCaptureProtocolAuthority &&
        (captureAuthority == null || captureAuthority.conversationId != conversationId)) {
      return false;
    }
    try {
  ...
  Future<void> _processNewSegmentReceived(
    ...
    if (segments.isEmpty && !_isLoadingInProgressConversation) {
      ...
      try {
        await refreshInProgressConversations(expectedTranscriptMomentGeneration: transcriptMomentGeneration);
      } catch (error) {
