...
void main() {
  setUpAll(() async {
    ...
    await (FontLoader(
      'MaterialIcons',
    )..addFont(materialIcons.readAsBytes().then((bytes) => ByteData.sublistView(Uint8List.fromList(bytes))))).load();
  });
  ...
  Future<void> pumpHardwareCard(
    ...
    await tester.runAsync(
      () => Future.wait(imageElements.map((element) => precacheImage((element.widget as Image).image, element))),
    );
