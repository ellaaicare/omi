...
def add_message(uid: str, message_data: dict, message_document_id: Optional[str] = None):
    ...
    if message_document_id:
        user_ref.collection('messages').document(message_document_id).set(message_data)
    else:
...
def add_app_message(
    ...
    message_id = (
        str(uuid.uuid5(uuid.NAMESPACE_URL, f'omi:external-integration-message:{idempotency_key}:{app_id}'))
        if idempotency_key
        else str(uuid.uuid4())
    )
    ai_message = Message(
