...
3. **LLM Prompt Instructions** (`utils/prompts.py` lines 48-64):
   ```
   - Limit: Identify up to 2 interesting memories and 2 system memories.
   - If there are none, output an empty list.
   - If you do not find any new (different to existing) or noteworthy facts,
     provide an empty list.
   - Most memories would be system memories. Interesting memories are RARE.
   - Interesting memories are only very interesting things the user would want
     to remember. If ambiguous, favor system memories.
   ```
