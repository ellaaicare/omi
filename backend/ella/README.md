...
- `ELLA_HERMES_PROVISION_ATTESTATION_KEY` is a distinct 32+ byte shared key used only for the provisioner's `honcho-isolation-v2` HMAC. Source rejects it when it equals any configured provision, callback, caregiver, event-ledger, dashboard, voice, operator, Honcho, or Hermes runtime/gateway credential. OMI sends a fresh UID/owner/target/binding/job challenge and rejects unsigned, replayed, stale, partial, or context-mismatched readback before publication.
...
- `ELLA_HERMES_PROVISION_API_TIMEOUT_SECONDS` bounds the cold-start request to 30-300 seconds and defaults to 180 seconds. The fixed proof lifetime is 360 seconds; the transport timeout plus `ELLA_HERMES_PROVISION_ATTESTATION_VERIFICATION_GRACE_SECONDS` (default 30 seconds) must be strictly less than 360 or provisioning fails before the request.
