...
def build_policy_markdown_view(
    user: UserPolicyContext,
    caregivers: list[CaregiverPolicyContext],
) -> str:
    """Render the effective escalation policy as agent-readable Markdown."""
    policy = build_plain_language_policy_view(user, caregivers)
    effective_policy = policy["effective_policy"]

    lines = [
        "# Ella Escalation Policy",
        "",
        "> Generated from the OMI backend structured policy. Do not edit this file by hand.",
        "",
        f"- policy_version: `{policy['policy_version']}`",
        f"- source: `{policy['source']}`",
        f"- uid: `{policy['uid']}`",
        f"- guardian_mode: `{policy['mode']}`",
        f"- channel_preference_version: `{policy['channel_preference_version']}`",
        "",
        "## Agent Contract",
        "",
        "- Wake words, direct questions, medication support, memory recall, and gentle guidance are user-first.",
        "- Do not send user-first events to caregivers unless a later critical safety event is produced.",
        "- Critical safety events notify the user and active emergency caregivers through allowed channels.",
        "- Reportable trends are saved for review or recap instead of immediate caregiver alerts.",
        "",
        "## Mode",
        "",
        policy["display"]["mode"],
        ...
        "## Delivery Rules",
        "",
        "| Rule | Event class | Decision | Target policy | Caregiver policy | Preferred channels |",
        "| --- | --- | --- | --- | --- | --- |",
    ]
    ...
    for rule in policy["delivery_rules"]:
        lines.append(
            ...
            + " | ".join(
                [
                    _markdown_cell(rule["rule_id"]),
                    f"`{_markdown_cell(rule['event_class'])}`",
                    f"`{_markdown_cell(rule['decision'])}`",
                    f"`{_markdown_cell(rule['target_policy'])}`",
                    f"`{_markdown_cell(rule['caregiver_policy'])}`",
                    ", ".join(f"`{_markdown_cell(channel)}`" for channel in rule["preferred_channels"]),
                ]
            )
            + " |"
        )
    ...
    lines.extend(
        [
            ...
            "## User Channels",
            "",
            "| Channel | Enabled | Reason |",
            "| --- | --- | --- |",
        ]
    )
    for channel in policy["user"]["channels"]:
        lines.append(
            "| "
            + " | ".join(
                [
                    f"`{_markdown_cell(channel['channel'])}`",
    ...
    lines.extend(
        [
            ...
            "## Emergency Contact",
            "",
            f"- configured: `{'true' if policy['emergency_contact']['configured'] else 'false'}`",
            f"- display_name: `{_markdown_cell(policy['emergency_contact']['display_name'] or '')}`",
            f"- status: `{_markdown_cell(policy['emergency_contact']['status'] or '')}`",
            f"- summary: {policy['emergency_contact']['text']}",
            "",
            "## Caregivers",
            "",
            "| Name | Status | Emergency contact | Emergency alerts | Daily summary | Weekly summary |",
            "| --- | --- | --- | --- | --- | --- |",
        ]
    )
    if policy["caregivers"]:
        for caregiver in policy["caregivers"]:
            permissions = caregiver["permissions"]
            lines.append(
                "| "
                + " | ".join(
                    [
                        _markdown_cell(caregiver["display_name"]),
                        f"`{_markdown_cell(caregiver['status'])}`",
                        "yes" if caregiver["is_emergency_contact"] else "no",
                        "yes" if permissions["emergency_alerts"] else "no",
                        "yes" if permissions["daily_summary"] else "no",
                        "yes" if permissions["weekly_summary"] else "no",
                    ]
                )
                + " |"
            )
    else:
        lines.append("| None configured |  | no | no | no | no |")

    caregiver_channels = ", ".join(
        f"`{_markdown_cell(channel)}`" for channel in effective_policy["caregiver_alert_preferences"]["channels"]
    )
    lines.extend(
        [
            "",
            ...
            "",
            f"- guardian_audio_enabled: `{'true' if effective_policy['guardian_audio_enabled'] else 'false'}`",
            f"- quiet_hours_active: `{'true' if effective_policy['quiet_hours_active'] else 'false'}`",
            (
                "- caregiver_emergency_contact_only: "
                f"`{'true' if effective_policy['caregiver_alert_preferences']['emergency_contact_only'] else 'false'}`"
            ),
            f"- caregiver_channels: {caregiver_channels}",
            "",
            "## Privacy Notes",
            "",
        ]
    )
    lines.extend(f"- {note}" for note in policy["privacy_notes"])
    lines.append("")
    return "\n".join(lines)
