...
MCP_TOOLS = [
    ...
    },
    {
        ...
        "description": "Update the structured summary fields of an existing conversation. All fields except conversation_id are optional - only provided fields will be updated.",
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "overview": {"type": "string", "description": "New summary/overview text"},
                "emoji": {"type": "string", "description": "New emoji for the conversation"},
                "category": {"type": "string", "description": "New category classification"},
            },
            "required": ["conversation_id"],
        },
    },
    ...
    if tool_name == "get_memories":
        ...
        for field in ("title", "overview", "emoji", "category"):
            ...
            if value is not None:
                update_data[f"structured.{field}"] = value

        if not update_data:
            raise ToolExecutionError(
                "At least one field (title, overview, emoji, category) must be provided"
            )
