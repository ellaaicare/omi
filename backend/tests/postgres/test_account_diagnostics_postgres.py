...
MIGRATIONS = tuple(
    ...
    for filename in (
        "018_create_account_diagnostics.sql",
        "019_add_diagnostic_attempt_ordinal.sql",
        "020_backfill_diagnostic_attempt_ordinals.sql",
    )
)
...
def test_two_accounts_cannot_cross_read_append_or_revoke_diagnostic_evidence():
    async def scenario() -> None:
        admin, pool, schema = await _create_schema()
        try:
            ...
            account_material = {}
            for suffix in ("a", "b"):
                uid = f"diagnostic-isolation-{suffix}"
                user_id = await pool.fetchval("INSERT INTO users (omi_uid) VALUES ($1) RETURNING id", uid)
                await pool.execute(
                    """
                    INSERT INTO ella_runtime_bindings (
                ...
                profile_binding_id = f"aipb_{suffix}"
                receipt_id = f"aicr_{suffix}"
                await pool.execute(
                    """
                    INSERT INTO ella_managed_cloud_consent_authority (
                        user_id, decision, consent_receipt_ref, profile_binding_id
            ...
            for suffix in ("a", "b"):
                material = account_material[suffix]
                event = _event().model_copy(
                    update={
                        "event_id": f"event-{suffix}",
                        "diagnostic_session_id": "shared-session-id",
                        "capture_attempt_id": f"attempt-{suffix}",
                        "account_binding_fingerprint": material["fingerprint"],
                    }
                )
                assert await repository.append_events(
                    material["authority"],
                    [event],
                    **material["authority_kwargs"],
                ) == (1, 0)
            ...
            for suffix in ("a", "b"):
                ...
                assert [event_from_record(record).event.event_id for record in records] == [f"event-{suffix}"]

            event_for_a_with_b_fingerprint = _event().model_copy(
                update={
                    "event_id": "cross-account-event",
                    "diagnostic_session_id": "cross-account-session",
                    "account_binding_fingerprint": account_material["b"]["fingerprint"],
                }
            )
            with pytest.raises(DiagnosticAccountAuthorityChanged):
                await repository.append_events(
                    account_material["a"]["authority"],
                    [event_for_a_with_b_fingerprint],
                    **account_material["a"]["authority_kwargs"],
                )
            ...
            assert (
                await repository.revoke_support_grant(
                    account_material["b"]["authority"],
                    grant_id,
                    **account_material["b"]["authority_kwargs"],
                )
                is False
            )
            assert await pool.fetchval(
                "SELECT revoked_at IS NULL FROM ella_diagnostic_support_grants WHERE id = $1::uuid",
                grant_id,
            )
            assert (
                await repository.revoke_support_grant(
