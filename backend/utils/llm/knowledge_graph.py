...
def extract_knowledge_from_memory(
    ...
    try:
        ...
        created_nodes = []
        for node_index, node in enumerate(extraction.nodes):
            ...
            node_id = existing_id or (
                document_id_from_seed(f'{idempotency_key}:knowledge-node:{node_index}')
                if idempotency_key
                else str(uuid.uuid4())
            )
        ...
        created_edges = []
        for edge_index, edge in enumerate(extraction.edges):
            ...
            if source_id and target_id:
                edge_data = {
                    'id': (
                        document_id_from_seed(f'{idempotency_key}:knowledge-edge:{edge_index}')
                        if idempotency_key
                        else None
                    ),
                    'source_id': source_id,
